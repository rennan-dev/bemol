...
class Produto {
    ...
    public static void OrdenaProdutoNome(List<Produto> produtos) {

        Console.Clear();
        Console.WriteLine("\n----------------------------------------");
        var ordenaPorNome = produtos.OrderBy(x => x.Nome).ToList();
        Console.WriteLine("Produtos Ordenados por nome:\n");
        foreach(var produto in ordenaPorNome) {
            Console.WriteLine($"Nome: {produto.Nome}");
            Console.WriteLine($"Descrição do produto: {produto.Descricao}");
            Console.WriteLine($"Preço unitário: {produto.PrecoUnitario:F2} R$");
            Console.WriteLine($"Quantidade no estoque: {produto.Quantidade}\n");
    ...
    public static void OrdenaProdutoPreco(List<Produto> produtos) {
        Console.Clear();
        Console.WriteLine("\n----------------------------------------");
        var ordenaPorPreco = produtos.OrderBy(x => x.PrecoUnitario).ToList();
        Console.WriteLine("Produtos Ordenados por nome:\n");
        foreach(var produto in ordenaPorPreco) {
            Console.WriteLine($"Nome: {produto.Nome}");
            Console.WriteLine($"Descrição do produto: {produto.Descricao}");
            Console.WriteLine($"Preço unitário: {produto.PrecoUnitario:F2} R$");
            Console.WriteLine($"Quantidade no estoque: {produto.Quantidade}\n");
